...
let eratosthenes n =
  let rec remove_multiples n list = match list with
    | [] -> []
    | e::l -> if modulo e n = 0 then
        remove_multiples n l
      else
        e::remove_multiples n l
  in let rec gen_r list = match list with
      | [] -> []
      | e::l when e > n -> []
      | e::l -> e::gen_r (remove_multiples e l)
  in gen_r (init_eratosthenes n);;
